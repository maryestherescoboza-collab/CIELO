    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <button
                ...
                onClick={disabled ? undefined : onChange}
                disabled={disabled}
                className={`relative inline-flex h-7.5 w-12.5 shrink-0 rounded-full border-2 border-transparent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[#FDFBF7] ${
                    disabled ? 'cursor-not-allowed bg-slate-200 opacity-50' : 'cursor-pointer ' + (checked ? 'bg-[#1E293B]' : 'bg-[#CBD5E1]')
                }`}
                style={{ transition: 'all 180ms ease' }}
            >
                <span
                    className={`pointer-events-none inline-block h-6.5 w-6.5 transform rounded-full bg-white shadow-sm ring-0 ${
