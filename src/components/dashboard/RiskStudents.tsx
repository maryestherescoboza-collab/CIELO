    ...
    return (
        <div className="card-saas bg-white p-0 overflow-hidden mb-8 shadow-sm border-slate-200">
            <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/10">
                <div>
                    <h2 className="flex items-center gap-3 text-[14px] font-bold uppercase tracking-[0.2em] text-cielo-low mb-1">
                        <TC_Anomaly size={18} className="text-cielo-low animate-pulse" /> Estudiantes en Riesgo
                    </h2>
                ...
                <div className="flex flex-col items-end">
                    <span className="text-[9px] font-black px-4 py-1 bg-cielo-low text-[#2E3330] rounded-full uppercase tracking-widest artisan-pill">
            ...
            {enRiesgo.length === 0 ? (
                <div className="py-24 flex flex-col items-center">
                    <div className="w-20 h-20 rounded-full bg-cielo-bg-main/5 border border-cielo-sidebar/10 flex items-center justify-center mb-6 group hover:border-turf-green-base/40 transition-all duration-700 shadow-inner">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-center gap-5">
                                        <div className="relative">
                                            ...
                                            <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-white rounded-full flex items-center justify-center border border-slate-100 shadow-md">
                                                <div className="w-2 h-2 rounded-full bg-[#EB8847] animate-ping"></div>
                                            </div>
                                        </div>
                                        <div>
                                    ...
                                    <div className="flex items-center gap-4 self-center md:self-auto">
                                        ...
                                        <div className="flex gap-2">
                                            <button className="rounded-full bg-cielo-sidebar text-cielo-on-background text-[10px] font-semibold tracking-widest hover:bg-[#D4CCBE] active:scale-95 transition-all artisan-pill artisan-btn-neutral" onClick={() => navigate(`/estudiante/${est.id}`)}>Perfil</button>
                                            <button className="rounded-full bg-cielo-sidebar text-cielo-on-background text-[10px] font-semibold tracking-widest hover:bg-[#D4CCBE] active:scale-95 transition-all artisan-pill artisan-btn-neutral" onClick={() => navigate(`/curso-detalle/${est.cursoId}`)}>Curso</button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {(studentIncidencias.length > 0 || studentCalificaciones.length > 0) && (
                                    <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:pl-19">
                                        {studentIncidencias.map(i => (
                                            <div key={`inc-${i.id}`} className={`p-4 rounded-xl border flex flex-col gap-2 transition-colors ${i.gravedad === 'grave' ? 'bg-[#EB8847]/10 border-cielo-low/20 text-[#2E3330]' : 'bg-cielo-accent2/10 border-cielo-accent2/20 text-[#2E3330]'}`}>
                                                <div className="flex items-center gap-2">
                                                    <span className={`w-2 h-2 rounded-full shrink-0 ${i.gravedad === 'grave' ? 'bg-[#EB8847]' : 'bg-[#F5BC5D]'}`}></span>
                                        ...
                                        {studentCalificaciones.map((c, idx) => {
                                            ...
                                            return (
                                                <div key={`calif-${idx}`} className="p-3 rounded-lg border border-[#EB8847]/25 bg-[#EB8847]/10 text-[#2E3330] flex flex-col gap-1.5 transition-colors">
                                                    <div className="flex items-center gap-2">
                                                        <span className="w-2 h-2 rounded-full bg-[#EB8847] shrink-0"></span>
                                                        <span className="text-[9px] font-black tracking-[0.2em] uppercase flex-1 truncate">Brecha de Logro</span>
                                                        <span className="text-[9px] font-black text-[#2E3330] bg-[#EB8847]/45 px-1.5 py-0.5 rounded-md">{c.puntaje}/100</span>
                                                    </div>
