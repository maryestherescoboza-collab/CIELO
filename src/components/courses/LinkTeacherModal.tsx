import { X, BookOpen, Search, Users } from 'lucide-react';
...
export function LinkTeacherModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-lg bg-white rounded-none shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-(--line) flex flex-col max-h-[80vh]">
                <div className="px-8 py-6 border-b border-(--line) flex items-center justify-between bg-(--paper) shrink-0">
                    <div>
                        <h2 className="text-xl font-black text-slate-900 tracking-tight">Vincular Docentes</h2>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Invita a otros maestros al curso</p>
                    </div>
                    <button className="btn-secondary p-2!" onClick={onClose}><X size={20} /></button>
                </div>

                <div className="p-8 overflow-y-auto flex-1">
                    <div className="mb-6 p-6 rounded-3xl bg-turf-green-base/5 border border-turf-green-base/20">
                        <h3 className="text-xs font-black text-turf-green-base uppercase tracking-widest mb-2">Compartir Enlace</h3>
                        <p className="text-[11px] text-slate-600 mb-4 font-medium">Invita a otros maestros directamente enviándoles este enlace de acceso rápido.</p>
                        <button onClick={copyLink} className="w-full py-3 bg-white border border-turf-green-base/30 rounded-xl text-turf-green-base text-[11px] font-bold shadow-sm hover:bg-turf-green-base hover:text-white transition-all flex items-center justify-center gap-2">
                        ...
                        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="bg-slate-100 border-none rounded-lg py-1 px-3 text-[10px] font-bold text-slate-700 w-40 focus:ring-2 focus:ring-slate-900 transition-all outline-none"
                                />
                                    ...
                                    return (
                                        <div key={p.userId} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 hover:border-slate-300 bg-slate-50/30 transition-all">
                                            ...
                                            <button
                                                ...
                                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isLinked
