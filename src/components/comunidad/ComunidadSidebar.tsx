    ...
    return (
        <aside className="space-y-5">
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2.5">
                    asignatura
                </h3>
                <div className="relative">
                    <select 
                        ...
                        className="w-full h-10 pl-4 pr-10 bg-[#F8F9FA] border border-slate-200/80 rounded-xl text-xs font-bold text-slate-500 focus:outline-none focus:border-slate-300 transition-all appearance-none cursor-pointer lowercase"
                        style={{
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 m-0">
                        top colaboradores
                    </h3>
                </div>
                    ...
                    {topColaboradores.map((profile, index) => (
                        ...
                        >
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="w-9 h-9 rounded-full overflow-hidden shadow-sm border border-slate-200 bg-white p-0.5 shrink-0">
                                    {profile.avatarUrl ? (
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className="text-xs font-bold text-[#1E293B] leading-tight mb-0.5 truncate">
                                        {profile.nombreDocente}
                                    </span>
                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 truncate max-w-30">{profile.asignatura?.toLowerCase()}</span>
                                </div>
                            </div>
                        ...
                        onlineUsers.map((user) => (
                            ...
                            >
                                <div className="relative shrink-0">
                                    <div className="w-9 h-9 rounded-full bg-slate-50 flex items-center justify-center overflow-hidden border border-slate-200 p-0.5 shadow-sm">
                                        {user.avatarUrl ? (
                                    ...
                                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-turf-green-base border-2 border-white rounded-full"></span>
                                </div>

                                <div className="flex flex-col min-w-0">
                                    <span className="text-xs font-bold text-[#1E293B] truncate leading-tight mb-0.5">{user.nombre}</span>
                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 italic truncate">
