    ...
    return (
        <div className={`w-full bg-white transition-all duration-300 ${isExpanded ? '' : ''}`}>
            <div className="flex flex-col gap-3">
                <div className="relative flex items-center bg-white rounded-[10px] p-3 border border-slate-300 group focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                    <textarea
                    ...
                    <div className="flex flex-col gap-3.5 pt-1 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">Categoría</label>
                                <select
                                    value={form.tipo}
                                    onChange={(e) => setForm({ ...form, tipo: e.target.value as Post['tipo'], recursoId: 0 })}
                                    className="w-full bg-white border border-slate-200 rounded-[10px] px-3 py-2 text-xs font-bold text-slate-700 outline-none transition-all cursor-pointer focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/50"
                                >
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">Asignatura</label>
                                <select
                        ...
                        {form.tipo !== 'general' && (
                            <div className="space-y-1">
                                <label className="text-xs font-black uppercase tracking-widest text-slate-400 px-1">Vincular Recurso</label>
                                <select
                                    value={form.recursoId || ''}
                                    onChange={(e) => setForm({ ...form, recursoId: e.target.value ? Number(e.target.value) : 0 })}
                                    className="w-full bg-white border border-slate-200 rounded-[10px] px-3 py-2 text-xs font-bold text-slate-700 outline-none transition-all focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/50"
                                >
                        ...
                        <div className="flex items-center justify-end gap-3 pt-1">
                            ...
                            <button
                                ...
                                className="bg-primary text-white px-6 py-2 rounded-[10px] text-xs font-black uppercase tracking-widest hover:bg-primary/90 disabled:opacity-50 disabled:hover:bg-primary transition-all flex items-center gap-2 shadow-lg shadow-primary/20 outline-none focus-visible:ring-2 focus-visible:ring-primary/50 hover:-translate-y-0.5 active:scale-95"
                            >
