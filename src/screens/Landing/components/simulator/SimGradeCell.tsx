        ...
        >
            <div className="flex items-center justify-center min-h-14 h-full relative overflow-visible">
                ...
                {isRecoveryAct ? (
                    ...
                    <div className={`rounded-full shadow-sm transition-transform hover:scale-110 ${score === null ? 'w-2 h-2 bg-[rgba(46,51,48,0.08)]' : score === 100 ? 'w-4 h-4 bg-primary' : score === 85 ? 'w-4 h-4 bg-attention' : score === 70 ? 'w-4 h-4 bg-danger' : score === 55 ? 'w-5 h-5 bg-[#2E3330]' : score >= 100 ? 'w-4 h-4 bg-primary' : score >= 85 ? 'w-4 h-4 bg-attention' : score >= 70 ? 'w-4 h-4 bg-danger' : 'w-5 h-5 bg-[#2E3330]'}`} />
                ) : (
