  ...
  return (
    <>
      ...
      {branch.children.map((child) => (
        ...
        >
          <div className="px-3 py-1.5 bg-white/90 rounded-full border border-[rgba(46,51,48,0.1)] shadow-sm text-xs md:text-sm text-[#5F665E] font-medium whitespace-nowrap transition-colors hover:bg-white hover:text-primary hover:border-primary/30 hover:shadow-md cursor-default">
            {child.label}
          </div>
        </div>
      ))}
        ...
        <div 
          className={`px-5 py-2.5 md:px-6 md:py-3 rounded-full border text-sm md:text-base font-semibold whitespace-nowrap transition-all duration-300
            ${isActive 
              ...
              : 'bg-white border-[rgba(46,51,48,0.1)] text-[#2E3330] shadow-sm hover:border-primary/50 hover:scale-105'
            }
          `}
        >
          {branch.label}
