...
export default function CentroPanel({ onLogout }: Props) {
    ...
    if (!centroId) {
        return (
            <div className="min-h-screen bg-(--background) text-(--ink) flex items-center justify-center p-6">
                <div className="bg-white border border-(--border-soft) rounded-(--radius-lg) p-10 text-center max-w-md shadow-sm">
                    <Building2 size={32} className="mx-auto text-(--primary) mb-4" />
                    <h2 className="text-[14px] font-black tracking-widest uppercase text-(--ink) mb-2">Panel no disponible</h2>
                    <p className="text-[11px] font-bold text-(--ink-soft) uppercase tracking-widest mb-6">
    ...
    if (!centro) {
        ...
        return (
            <div className="min-h-screen bg-(--background) text-(--ink) flex items-center justify-center p-6">
                <div className="bg-white border border-(--border-soft) rounded-(--radius-lg) p-10 text-center max-w-md shadow-sm">
                    <AlertTriangle size={32} className="mx-auto text-(--danger) mb-4" />
                    <h2 className="text-[14px] font-black tracking-widest uppercase text-(--ink) mb-2">Error de Sincronización</h2>
                    <p className="text-[11px] font-bold text-(--ink-soft) uppercase tracking-widest mb-6">
        ...
        <div className="min-h-screen bg-(--background) text-(--ink) font-sans flex flex-col selection:bg-(--primary)/20">
            ...
            <header className="px-6 py-4 border-b border-(--border-soft) flex flex-col md:flex-row md:items-center justify-between bg-(--background) sticky top-0 z-40 gap-4">
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button onClick={onLogout} className="text-[9px] font-black uppercase tracking-[0.15em] text-(--terra-cotta) hover:text-(--ink) transition-colors bg-white/50 px-3 py-1.5 rounded-[8px] border border-(--border-soft) hover:bg-white shadow-sm cursor-pointer">
                        Cerrar Sesión
                    </button>
                </div>
            </header>
