...
export default function Rubrica({
    ...
    function handleImportMarkdown(text: string) {
        const lines = text
            .split('\n')
            .map(line => line.trim())
        ...
        if (lines.length < 2) {
            alert('Formato inválido. Debe contener una cabecera y al menos una fila de competencia.');
            return;
        }
        ...
        const requiredHeaders = ['Competencia', 'Estratégico', 'Autónomo', 'Resolutivo', 'Receptivo'];
        const hasAllRequired = requiredHeaders.every(req => 
            headerCells.some(h => h.toLowerCase().includes(req.toLowerCase()))
        );

        if (!hasAllRequired || headerCells.length < 5) {
            alert('Formato inválido. La tabla debe contener exactamente estas cinco columnas:\nCompetencia | Estratégico | Autónomo | Resolutivo | Receptivo');
            return;
        }

        const hasExistingData = localDescriptors.some(d => d.estrategico || d.autonomo || d.resolutivo || d.receptivo);
        if (hasExistingData) {
            const confirmReplace = window.confirm('La rúbrica actual ya contiene información. ¿Deseas sustituirla con los datos importados?');
            if (!confirmReplace) return;
        }

        const parsedRows: { bc: string; estrategico: string; autonomo: string; resolutivo: string; receptivo: string }[] = [];
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const bcName = cells[0].toUpperCase();
            if (['BC1', 'BC2', 'BC3', 'BC4'].includes(bcName)) {
                parsedRows.push({
                    bc: bcName,
                    estrategico: cells[1],
                    autonomo: cells[2],
        ...
        if (parsedRows.length === 0) {
            alert('No se encontraron filas con competencias válidas (BC1, BC2, BC3 o BC4) para importar.');
            return;
        }

        setLocalDescriptors(prev => {
            return prev.map(descriptor => {
                const match = parsedRows.find(row => row.bc === descriptor.bc);
                if (match) {
                    return {
                        ...descriptor,
                        estrategico: match.estrategico,
                        autonomo: match.autonomo,
                        resolutivo: match.resolutivo,
                        receptivo: match.receptivo
                    };
                }
                return descriptor;
            });
