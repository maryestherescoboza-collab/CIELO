    ...
    <div className="min-h-screen relative flex flex-col justify-center items-center px-6 overflow-hidden bg-(--paper) font-body">
      ...
      <main className="w-full max-w-md relative z-10 py-12">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="w-48 h-48 md:w-56 md:h-56 p-4 bg-white rounded-3xl shadow-paper border border-(--line) flex items-center justify-center mb-4">
          ...
          <div className="text-center">
            ...
            <p className="text-xs font-black uppercase tracking-[0.25em] text-(--ink-soft) opacity-40">Restablecer Contraseña</p>
          </div>
        </div>
          ...
          {isValidating ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5 animate-fade-in">
              <div className="mb-4">
                ...
                <p className="text-xs font-bold text-(--ink-soft) opacity-40 uppercase tracking-wider mt-1">
                  Hola {docenteNombre}, ingresa tus nuevas credenciales
                </p>
              </div>

              {error && (
                <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl text-xs font-bold text-red-600 flex items-center gap-2">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-(--ink-soft) uppercase tracking-widest ml-1 opacity-40">Nueva Contraseña</label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-(--ink-soft) uppercase tracking-widest ml-1 opacity-40">Confirmar Nueva Contraseña</label>
                <div className="relative group">
              ...
              <div className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-2.5">
                <p className="text-xs font-black uppercase tracking-widest text-(--ink-soft) opacity-40">Requisitos de Seguridad</p>
                <div className="grid grid-cols-2 gap-2">
                  <div className={`flex items-center gap-2 text-xs font-bold ${matchesLength ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {matchesLength ? <Check size={12} /> : <span className="w-1.5 h-1.5 bg-slate-300 rounded-full ml-1"></span>}
                    Mínimo 8 caracteres
                  </div>
                  <div className={`flex items-center gap-2 text-xs font-bold ${hasUppercase ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {hasUppercase ? <Check size={12} /> : <span className="w-1.5 h-1.5 bg-slate-300 rounded-full ml-1"></span>}
                    Una letra mayúscula
                  </div>
                  <div className={`flex items-center gap-2 text-xs font-bold ${hasLowercase ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {hasLowercase ? <Check size={12} /> : <span className="w-1.5 h-1.5 bg-slate-300 rounded-full ml-1"></span>}
                    Una letra minúscula
                  </div>
                  <div className={`flex items-center gap-2 text-xs font-bold ${hasNumber ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {hasNumber ? <Check size={12} /> : <span className="w-1.5 h-1.5 bg-slate-300 rounded-full ml-1"></span>}
                    Al menos un número
                  </div>
                </div>
                {confirmPassword && (
                  <div className={`pt-2 border-t border-slate-100 flex items-center gap-2 text-xs font-bold ${matchesConfirm ? 'text-emerald-600' : 'text-red-500'}`}>
              ...
              <button
                ...
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-xs tracking-[0.2em] shadow-xl shadow-emerald-900/10 transition-all active:scale-[0.98] disabled:opacity-40 flex items-center justify-center gap-3 uppercase"
              >
                {submitting ? (
        ...
        {/* Brand Meta */}
        <p className="mt-8 text-center text-xs font-bold text-(--ink-soft) opacity-20 uppercase tracking-[0.3em]">Noether v2.4.0 • 2026 Edition</p>
      </main>
    </div>
  );
}
